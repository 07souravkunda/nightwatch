...
class Browserstack extends BaseDriver {
  ...
  adaptSettings() {
    ...
    if (process.env.ADDITIONAL_BSTACK_CAPS) {
      try {
        const additionalBstackCaps = JSON.parse(process.env.ADDITIONAL_BSTACK_CAPS);
        Object.keys(additionalBstackCaps).forEach(cap => {
          this.settings.desiredCapabilities['bstack:options'][cap] = additionalBstackCaps[cap];
        })
      }catch(_) {
        Logger.error('ADDITIONAL_BSTACK_CAPS is invalide');
      }
    }
